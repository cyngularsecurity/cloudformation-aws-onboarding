...
## Part 1: Deploy Management Account CloudFormation Stacks
...
- Parameters (align with your Core parameters):
  - ClientName = <`client company name`>
  - ClientAccountId = <`management account ID`> (required when deploying in organizations)
  ...
  - ExcludedRegions = comma-separated region list (optional)
...
- Deployment options:
  - Maximum concurrent accounts: Percentage - 90
  - Failure tolerance: Percentage - 70
...
- Deployment options (optional tuning):
...
- If you use your own S3 buckets (e.g., CloudTrail, VPC Flow Logs), ensure required tags and bucket policies are applied. See [Service Configuration](./SERVICE_CONFIGURATION.md).
