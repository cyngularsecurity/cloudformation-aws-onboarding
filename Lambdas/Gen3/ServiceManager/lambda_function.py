...
class ServiceManager:
    ...
    def __init__(self, lambda_context):        
        ...
        self.region_processor_function = os.environ['REGIONAL_SERVICE_MANAGER_FUNCTION_NAME']
    ...
    def get_enabled_regions(self) -> List[str]:
        ...
        try:
            ...
            logger.warning(f"[{self.fallback_lambda_region} | ServiceManager] Falling back to current region: {current_lambda_region}")
        ...
        with ThreadPoolExecutor(max_workers=self.MAX_CONCURRENT_WORKERS) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    logger.error(f"[{region} | ServiceManager] Task {service}/{region} failed with {error_details['error_type']}: {error_details['error_message']}")
    ...
    def handle_cloudformation_event(self, event: Dict[str, Any], context: Any) -> None:
        ...
        try:
            ...
            error_details = {
                ...
                'stack_id': event.get('StackId', 'Unknown')[:100] if event.get('StackId') else 'Unknown'
            }
            
            logger.error(f"[{self.fallback_lambda_region} | ServiceManager] CloudFormation event handling failed: {error_details['error_type']} - {error_details['error_message']}")
            logger.error(f"[{self.fallback_lambda_region} | ServiceManager] Event context: RequestType={error_details['request_type']}, StackId={error_details['stack_id']}")
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"[{fallback_region} | ServiceManager] Lambda handler failed: {error_details['error_type']} - {error_details['error_message']}")
        logger.error(f"[{fallback_region} | ServiceManager] Event type: {error_details['event_type']}")
        
        # Record error metrics if we have service_manager instance
        try:
            if 'service_manager' in locals():
                service_manager.metrics.record_error(error_details['error_type'], error_details['error_message'])
        except Exception:
