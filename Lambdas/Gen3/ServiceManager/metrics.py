...
class MetricsCollector:
    ...
    def put_metric(self, namespace: str, metric_name: str, value: float, unit: str = 'Count', 
        ...
        try:
            ...
            logger.warning(f"Failed to send metric {namespace}/{metric_name}: {error_code} - {str(e)}")
        except Exception as e:
            logger.warning(f"Unexpected error sending metric {namespace}/{metric_name}: {str(e)}")
    ...
    def put_metrics_batch(self, namespace: str, metrics: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            error_code = e.response['Error']['Code']
            logger.warning(f"Failed to send batch metrics to {namespace}: {error_code} - {str(e)}")
        except Exception as e:
            logger.warning(f"Unexpected error sending batch metrics to {namespace}: {str(e)}")
    
    def record_processing_results(self, results: Dict[str, Any], namespace: str = 'Cyngular/Lambda') -> None:
        """
        ...
        if 'services_done' in results:
            metrics.append({'name': 'TasksSuccessful', 'value': results['services_done']})
            
        if 'services_failed' in results:
            metrics.append({'name': 'TasksFailed', 'value': results['services_failed']})
            
        if 'success_rate' in results:
            metrics.append({'name': 'SuccessRate', 'value': results['success_rate'], 'unit': 'Percent'})
            
        if 'processing_time_seconds' in results:
            metrics.append({'name': 'ProcessingTime', 'value': results['processing_time_seconds'], 'unit': 'Seconds'})
    ...
    def record_error(self, error_type: str, error_message: str, namespace: str = 'Cyngular/Lambda') -> None:
        """
    ...
    def record_invocation(self, event_type: str = 'Unknown', namespace: str = 'Cyngular/Lambda') -> None:
        """
