...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"RegionProcessor handler failed: {error_details['error_type']} - {error_details['error_message']}")
        logger.error(traceback.format_exc())  # Log full traceback for debugging
        
        # Record error metrics if we have processor instance
        try:
            if 'processor' in locals():
                processor.metrics.record_error(error_details['error_type'], error_details['error_message'])
        except Exception:
