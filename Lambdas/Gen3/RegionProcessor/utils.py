...
def process_eks_service(region: str, cyngular_role_arn: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f'[{region} | EKS] Found {len(clusters)} clusters in region {region}')
        ...
        for cluster_name in clusters:
            try:
                ...
                if all(log_type in current_types for log_type in wanted_types):
                    logger.info(f'[{region} | EKS] Cluster {cluster_name} already has required logging enabled')
                else:
                    logger.info(f'[{region} | EKS] Updating logging configuration for {cluster_name}')
                    ...
                    try:
                        ...
                        logger.info(f'[{region} | EKS] Successfully updated logging for {cluster_name}')
                    except ClientError as e:
                        if 'No changes needed for the logging config provided' in str(e):
                            logger.info(f'[{region} | EKS] No changes needed for logging config in {cluster_name}')
                        else:
                            raise e
                ...
                logger.error(f'[{region} | EKS] Error processing cluster {cluster_name}: {str(e)}')
                processed_clusters.append({
                    'cluster': cluster_name,
                    'error': str(e)
                })
...
def process_os_service(region: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not instance_ids:
            logger.info(f'[{region} | OS INTERNALS] No running instances found in {region}')
        ...
        for instance_id in instance_ids:
            try:
                logger.info(f'[{region} | OS INTERNALS] CONFIGURING INSTANCE-ID: {instance_id}')
                ...
                logger.info(f'[{region} | OS INTERNALS | {instance_id}] COMMAND SUCCEEDED')
                ...
                if e.response['Error']['Code'] == 'InvalidInstanceId' and 'not in a valid state for account' in str(e):
                    logger.warning(f'[{region} | OS INTERNALS | {instance_id}] COMMAND FAILED: {str(e)}')
                    processed_instances.append({
                        'instance_id': instance_id,
                        'error': str(e)
                    })
                ...
                logger.error(f'[{region} | OS INTERNALS | {instance_id}] COMMAND FAILED: {str(e)}')
                processed_instances.append({
                    'instance_id': instance_id,
                    'error': str(e)
                })
        ...
        logger.error(f'[{region} | OS INTERNALS | ClientError] OS processing failed: {str(e)}')
        return {'success': False, 'error': str(e)}
    except Exception as e:
